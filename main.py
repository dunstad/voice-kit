...
def toggle_tv_power():
    say('Switching TV power.')
    subprocess.call('node /home/pi/voice-recognizer-raspi/src/tv-control/toggle.js', shell=True)

def play_youtube(query):
    say('Playing {0}.'.format(query))
    subprocess.call('node /home/pi/voice-recognizer-raspi/src/tv-control/youtube.js {0}'.format(query), shell=True)

def set_volume(volume):
    say('Setting volume to {0}.'.format(volume))
...
def process_event(assistant, event):
    ...
    if event.type == EventType.ON_START_FINISHED:
        ...
        if text == 'power off':
            ...
            assistant.stop_conversation()
            play_youtube(text[text.index(' ') + 1:])
        elif all(word in text for word in ['set', 'volume']):
            assistant.stop_conversation()
            set_volume(text.split(' ')[-1])
